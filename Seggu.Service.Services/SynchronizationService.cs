...
namespace Seggu.Service.Services
{
    public class SynchronizationService : ISynchronizationService
    {
        ...
        public void MarkEnd()
        {
            var syncs = this.context.SynchronizationProcesses;
            this.context.SynchronizationProcesses.RemoveRange(syncs);
            this.context.SaveChanges();
        }

        public void MarkStart()
        {
            var sync = this.context.SynchronizationProcesses.FirstOrDefault();
            if (sync == null)
            {
                sync = new SynchronizationProcess();
            }
            sync.Date = DateTime.UtcNow;
            sync.Type = SynchronizationProcessType.IN_PROGRESS;
            sync.Message = "Sincronizacion en progreso.";
            this.context.SynchronizationProcesses.Add(sync);
            this.context.SaveChanges();
        }
        ...
        private bool IsPaid()
        {
            ...
            if (!string.IsNullOrEmpty(paidAt.ToString()))
                ok = DateTime.Now - paidAt < TimeSpan.FromDays(30);
            else
                ok = false;
